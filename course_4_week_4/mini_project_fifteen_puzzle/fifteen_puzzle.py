...
from poc_imports import poc_fifteen_gui
...
class Puzzle:
    ...
    def gen_zero_move_string(self, target_row, target_col):
        ...
        x_dist = target_tile_loc[0] - target_row
        y_dist = target_tile_loc[1] - target_col
        if x_dist > 0:
    ...
    def solve_interior_tile(self, target_row, target_col):
        ...
        # move to col first then row

        # if dest_row = curr_row move right ""
        # if dest_row > curr_row move down

        # if dest_col = curr_col move down
        # if dest_col > curr_col move right
        # if dest_col < curr_col move left

        assert self.lower_row_invariant(target_row, target_col)

        # find location of target tile and generate move string for
        # zero to that location

        move_string += self.gen_zero_move_string(target_row, target_col)

        # detect relative postion of zero to target tile
        # target new position of zero if last move u = top, l = left, r = right
        move_string += relative_pos.move_string[-1]

        # move tile back to target tile
